...
/// 智能助手配置：OpenAI 兼容协议（chat/completions），可接任意免费模型端点。
/// 所有配置只存在本机 start_prefs，不收集、不上传其他数据。
///
/// 这一层同时是 2.0 重构的服务接口预留：核心功能（记一条、排日程、转念头）
/// 统一通过 [Assistant.apply] 落库，UI 与解耦后的执行层以后可直接复用。
class AiConfig {
  AiConfig._();
...
你是一个待办 App 的意图解析器。把用户的一句话解析为一个动作，只输出 JSON，禁止输出任何解释或 Markdown 代码块。
...
- 拿不准具体时间时按 task 处理，due 留空。''';
  ...
  /// 自然语言 → 结构化意图。模型异常时抛出，调用方自行提示。
  static Future<AssistIntent> parse(String spoken) async {
    ...
    if (match == null) throw Exception('模型未返回 JSON');
    final m = jsonDecode(match.group(0)!) as Map<String, dynamic>;
  ...
  /// 执行动作并返回新建条目（none 或标题空时返回 null）。
  static Future<Item?> apply(AssistIntent intent) async {
  ...
  /// 一步到位：说话/文字 → 解析 → 落库。
  static Future<Item?> handle(String text) async => apply(await parse(text));
}
